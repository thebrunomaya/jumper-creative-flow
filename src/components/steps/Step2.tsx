...
const Step2: React.FC<Step2Props> = ({
  ...
  if (formData.creativeType === 'carousel') {
    ...
    return <div className="space-y-8 animate-fade-in">
        ...
        {/* Aspect Ratio Toggle */}
        <CarouselAspectRatioToggle value={carouselAspectRatio} onValueChange={updateCarouselAspectRatio} />

        {/* Carousel Cards */}
        <div className="space-y-6">
          {carouselCards.map((card, index) => <CarouselCardUpload key={card.id} card={card} aspectRatio={carouselAspectRatio} onFileChange={file => updateCardFile(card.id, file)} onRemove={index >= 2 ? () => removeCard(card.id) : undefined} canRemove={index >= 2} cardNumber={index + 1} />)}
        </div>

        {/* Add Card Button */}
        {carouselCards.length < CAROUSEL_LIMITS.maxCards && <div className="text-center">
            <Button variant="outline" onClick={addCard} className="bg-white hover:bg-gray-50 border-2 border-dashed border-gray-300 hover:border-jumper-blue transition-colors">
        ...
        {/* Validation Errors */}
        {errors.files && <div className="bg-red-50 border border-red-200 rounded-lg p-4">
  ...
  if (formData.creativeType === 'single') {
    ...
    const hasAnyDisabledPosition = () => {
      return mediaVariations.some(variation => !variation.squareEnabled || !variation.verticalEnabled || !variation.horizontalEnabled);
    };
    return <div className="space-y-8 animate-fade-in">
        <div className="text-center mb-8">
        ...
        {hasAnyDisabledPosition() && <Alert className="border-yellow-200 bg-yellow-50">
        ...
        {mediaVariations.map((variation, index) => <div key={variation.id} className="space-y-6 p-6 rounded-lg bg-white">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-jumper-text">
                📎 Mídia {variation.id}
              </h3>
              {mediaVariations.length > 1 && <Button variant="outline" size="sm" onClick={() => removeVariation(variation.id)} className="text-red-600 hover:text-red-700 hover:bg-red-50">
            ...
            <div className="grid gap-6">
              <SingleFileUploadSection title="📐 Formato Quadrado" format="square" dimensions="1080x1080px ou múltiplos superiores (1:1)" file={variation.squareFile} onFileChange={file => updateVariationFile(variation.id, 'square', file)} enabled={variation.squareEnabled !== false} onEnabledChange={enabled => updateVariationEnabled(variation.id, 'square', enabled)} canDisable={canDisablePosition(variation, 'square')} />

              <SingleFileUploadSection title="📱 Formato Vertical" format="vertical" dimensions="1080x1920px ou múltiplos superiores (9:16)" file={variation.verticalFile} onFileChange={file => updateVariationFile(variation.id, 'vertical', file)} enabled={variation.verticalEnabled !== false} onEnabledChange={enabled => updateVariationEnabled(variation.id, 'vertical', enabled)} canDisable={canDisablePosition(variation, 'vertical')} />

              <SingleFileUploadSection title="💻 Formato Horizontal" format="horizontal" dimensions="1200x628px ou múltiplos superiores (1.91:1)" file={variation.horizontalFile} onFileChange={file => updateVariationFile(variation.id, 'horizontal', file)} enabled={variation.horizontalEnabled !== false} onEnabledChange={enabled => updateVariationEnabled(variation.id, 'horizontal', enabled)} canDisable={canDisablePosition(variation, 'horizontal')} />
            </div>
          </div>)}

        {/* Add Variation Button */}
        {mediaVariations.length < 10 && <div className="text-center">
            <Button variant="outline" onClick={addVariation} className="bg-white hover:bg-gray-50 border-2 border-dashed border-gray-300 hover:border-jumper-blue transition-colors">
              <Plus className="h-4 w-4 mr-2" />
  ...
  return <div className="space-y-6 animate-fade-in">
      ...
      <FileUpload files={formData.validatedFiles} onFilesChange={files => updateFormData({
      ...
      {formData.validatedFiles.length > 0 && <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
          <div className="flex items-center space-x-2">
            <div className="flex-shrink-0">
              <svg className="h-5 w-5 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
